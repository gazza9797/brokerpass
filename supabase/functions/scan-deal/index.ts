...
2. INITIALS AND SIGNATURES AUDIT (do this before judging any rule): go through the package page by page. For every page that has per-page initial boxes (the "INITIALS OF BUYER(S):" / "INITIALS OF SELLER(S):" ovals at the foot of OREA forms), record for that page which boxes contain marks (initials, e-signature stamps, scribbles) and which are EMPTY (a bare oval or blank line with nothing in it). Do the same for every signature block. Base BP-PKG-01 and BP-PKG-02 strictly on this audit: one empty required box on one page = failed, even if every other page is complete. Do not assume a page is initialed because the others are.
